...
AS $$
  ...
  -- Return benchmark rows if requested
  SELECT 
    bp.asset_class_id,
  ...
  ORDER BY is_benchmark ASC, ticker ASC;
$$;

GRANT EXECUTE ON FUNCTION public.get_asset_class_table(date, uuid, boolean) TO anon, authenticated, service_role;

-- 3. get_scores_as_of - Placeholder that returns basic fund data (real scoring comes later)
...
AS $$
  ...
  WHERE (f.asset_class_id = p_asset_class_id OR p_asset_class_id IS NULL)
