...
-- Consolidated manual SQL script
...
-- Single-default constraint via partial unique index
...
-- 5) Seed default profile and weights (idempotent)
-- Ensure a default profile named 'Default' exists
...
-- Ensure only one default (move others off default if needed)
with ranked as (
...
-- Upsert DEFAULT_WEIGHTS exactly
-- Metric set mirrors src/services/scoring.js DEFAULT_WEIGHTS
-- Keys and weights must match exactly
with def as (
