...
import { getAllCombinedSnapshots } from "./services/enhancedDataStore";
      ...
      {METRIC_ORDER.filter((m) => breakdown[m]).map((metric) => {
        ...
        return (
          ...
          >
            ...
            <span
              style={{
                color: data.weightedZScore >= 0 ? "#16a34a" : "#dc2626",
              }}
            >
              {data.weightedZScore >= 0 ? "+" : ""}
              {data.weightedZScore.toFixed(3)}
            </span>
          </div>
        );
      })}
...
const App = () => {
  ...
  useEffect(() => {
    const handleKeyPress = (e) => {
      ...
      if (
        ...
        downloadFile(
          blob,
          `fund_analysis_${new Date().toISOString().split("T")[0]}.xlsx`,
        );
      }
  ...
  const loadSnapshots = async () => {
    try {
      ...
      const historicalCount = allSnapshots.filter(
        (s) => s.metadata && s.metadata.source === "historical_data",
      ).length;
      const userCount = allSnapshots.length - historicalCount;
      console.log(
    ...
    reader.onload = async (e) => {
      try {
        ...
        let headerRowIndex = jsonData.findIndex((row) =>
          row.some(
            (cell) => typeof cell === "string" && cell.includes("Symbol"),
          ),
        );
        ...
        headers.forEach((header, index) => {
          if (typeof header === "string") {
            ...
            if (
              ...
              headerLower.includes("cusip")
            ) {
              columnMap["Symbol"] = index;
            }
            if (
              headerLower.includes("product name") ||
              headerLower === "name"
            ) {
            ...
            if (
              ...
              (headerLower.includes("category rank") &&
                headerLower.includes("total return") &&
                (headerLower.includes("1y") ||
                  headerLower.includes("1 year")) &&
            ...
            // Capture ratios
            if (
              headerClean === "Up Capture Ratio (Morningstar Standard) - 3 Year"
            ) {
        ...
        const parsed = dataRows
          .map((row) => {
            ...
            if (fund["Standard Deviation"] != null) {
              if (fund["StdDev3Y"] == null)
                fund["StdDev3Y"] = fund["Standard Deviation"];
              if (fund["StdDev5Y"] == null)
                fund["StdDev5Y"] = fund["Standard Deviation"];
            }
        ...
        const withClassAndFlags = parsed.map((f) => {
          ...
          const recommendedMatch = recommendedFunds.find(
            (r) => clean(r.symbol) === parsedSymbol,
          );
          ...
          Object.entries(assetClassBenchmarks).forEach(
        ...
        const benchmarks = {};
        Object.entries(assetClassBenchmarks).forEach(
          ([assetClass, { ticker, name }]) => {
            const match = scoredFunds.find(
              (f) => f.cleanSymbol === clean(ticker),
            );
            if (match) {
        ...
        const dateStr = prompt(
          "Enter the date for this snapshot (YYYY-MM-DD):",
        ...
        console.log(
          "Successfully loaded and scored",
          scoredFunds.length,
          "funds",
        );
      } catch (err) {
  ...
  const loadSnapshot = async (snapshot) => {
    ...
    const fundsWithGroup = snapshot.funds.map((f) => ({
      ...
      displayName:
        registryNameMap[f.cleanSymbol] || f.displayName || f["Fund Name"],
    }));
      ...
      ([assetClass, { ticker, name }]) => {
        const clean = (s) =>
          s
            ...
            .trim()
            .replace(/[^A-Z0-9]/g, "");
        const match = snapshot.funds.find(
          (f) => f.cleanSymbol === clean(ticker),
        );
        if (match) {
          benchmarks[assetClass] = { ...match, name };
  ...
  const handleCompareSnapshots = async () => {
    ...
    try {
      const comparison = await compareSnapshotsAPI(
        selectedSnapshot.id,
        compareSnapshot.id,
      );
    ...
    >
        ...
        <div>
          {scoredFundData.length > 0 ? (
            <>
                ...
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  <button
                    onClick={() => {
                      const data = {
                        ...
                        classSummaries,
                        reviewCandidates:
                          identifyReviewCandidates(scoredFundData),
                        metadata: {
                  ...
                  <button
                    onClick={() => {
                      ...
                      downloadFile(
                        csv,
                        `fund_scores_${new Date().toISOString().split("T")[0]}.csv`,
                        "text/csv",
                      );
                    }}
                    style={{
              ...
              <TopBottomPerformers funds={scoredFundData} />
          ...
          {scoredFundData.length > 0 ? (
            <div>
              <div style={{ marginBottom: "1rem" }}>
                ...
                <p style={{ color: "#6b7280", fontSize: "0.875rem" }}>
                  Scores calculated using weighted Z-score methodology within
                  each asset class
                </p>
              </div>
                      ...
                      .map((fund, i) => (
                        <tr
                          ...
                          style={{
                            ...
                            backgroundColor: fund.isRecommended
                              ? "#eff6ff"
                              : "white",
                            cursor: "pointer",
                          }}
                        ...
                        >
                          <td
                            style={{
                              padding: "0.75rem",
                              fontWeight: fund.isBenchmark ? "bold" : "normal",
                            }}
                          >
                            {fund.Symbol}
                          </td>
                    ...
                    <div>
                      ...
                      >
                        <span style={{ color: "#16a34a" }}>
                          {classSummaries[selectedClass].distribution.excellent}{" "}
                          excellent
                        </span>
                ...
                <tbody>
                  {benchmarkData[selectedClass] && (
                    ...
                    >
                      ...
                      <td style={{ padding: "0.75rem" }}>
                        {benchmarkData[selectedClass].displayName ||
                          benchmarkData[selectedClass]["Fund Name"] ||
                          benchmarkData[selectedClass].name}
                      </td>
                      <td style={{ padding: "0.75rem", textAlign: "center" }}>
                        {benchmarkData[selectedClass].scores ? (
                          <ScoreBadge
                            score={benchmarkData[selectedClass].scores.final}
                          />
                        ) : (
                          "-"
                        )}
                      ...
                      <td style={{ padding: "0.75rem", textAlign: "right" }}>
                        {benchmarkData[selectedClass]["Sharpe Ratio"]?.toFixed(
                          2,
                        ) ?? "-"}
                      </td>
                      <td style={{ padding: "0.75rem", textAlign: "right" }}>
                  ...
                  {scoredFundData
                    .filter(
                      (f) =>
                        f["Asset Class"] === selectedClass && !f.isBenchmark,
                    )
                    .sort(
                ...
                {reviewCandidates.length > 0 ? (
                  <>
                    ...
                    >
                      <p style={{ fontWeight: "500" }}>
                        ...
                        />
                        {reviewCandidates.length} funds flagged for review based
                        on scoring criteria
                      </p>
                    </div>
                  ...
                  >
                    ...
                    <p style={{ color: "#6b7280", marginTop: "0.5rem" }}>
                      No funds currently flagged for review based on scoring
                      criteria.
                    </p>
                  </div>
                )}
              ...
              >
                ...
                {(() => {
                  const diversification =
                    calculateDiversification(scoredFundData);
                    ...
                    >
                      <div>
                        ...
                        <div style={{ fontSize: "0.875rem" }}>
                          <div>
                            Asset Classes:{" "}
                            <strong>{diversification.assetClassCount}</strong>
                          </div>
                          <div>
                            Effective Classes:{" "}
                            <strong>
                              {diversification.effectiveAssetClasses}
                            </strong>
                          </div>
                          <div>
                            Concentration Risk:{" "}
                        ...
                        <div style={{ fontSize: "0.875rem" }}>
                          <div>
                            ...
                            <strong style={{ color: "#16a34a" }}>
                              {diversification.scoreDistribution.excellent}
                            </strong>
                          </div>
                          <div>
                            Good (50-70):{" "}
                            <strong style={{ color: "#eab308" }}>
                              {diversification.scoreDistribution.good}
                            </strong>
                          </div>
                          <div>
                            Poor (&lt;50):{" "}
                            <strong style={{ color: "#dc2626" }}>
                              {diversification.scoreDistribution.poor}
                            </strong>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: "0.875rem" }}>
                          <div>
                            Performance:{" "}
                            <strong>{outliers.performance.length}</strong>
                          </div>
                          <div>
                            Risk: <strong>{outliers.risk.length}</strong>
                          </div>
                          <div>
                            Expense: <strong>{outliers.expense.length}</strong>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: "0.875rem" }}>
                          <div>
                            Domestic:{" "}
                            <strong>
                              {diversification.geographicDiversity.domestic}
                            </strong>
                          </div>
                          <div>
                            International:{" "}
                            <strong>
                              {
                                diversification.geographicDiversity
                                  .international
                              }
                            </strong>
                          </div>
                          <div>
                            Emerging:{" "}
                            <strong>
                              {diversification.geographicDiversity.emerging}
                            </strong>
                          </div>
                        </div>
                      </div>
                                ...
                                >
                                  <span
                                    style={{
                                      ...
                                      borderRadius: "0.25rem",
                                      backgroundColor:
                                        getScoreColor(data.avgScore) + "20",
      ...
      {activeTab === "history" && (
        <div>
          ...
          <FundTimeline snapshots={snapshots} currentFunds={scoredFundData} />
              ...
              {snapshotComparison && (
                <div style={{ marginBottom: "1.5rem" }}>
                  <h3 style={{ fontWeight: "bold", marginBottom: "0.5rem" }}>
                    Score Changes:{" "}
                    {new Date(
                      snapshotComparison.snapshot1.date,
                    ).toLocaleDateString()}{" "}
                    →{" "}
                    {new Date(
                      snapshotComparison.snapshot2.date,
                    ).toLocaleDateString()}
                  </h3>
                  <div style={{ overflowX: "auto" }}>
                    <table
          ...
          >
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              >
                ...
                <li>
                  The system will automatically calculate Z-scores for each fund
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              >
                Each fund receives a 0-100 score based on weighted Z-scores
                across 13 metrics:
              </p>
              <div
                style={{
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              <div style={{ fontSize: "0.875rem", color: "#374151" }}>
                <div style={{ marginBottom: "0.25rem" }}>
                  <kbd>Ctrl+H</kbd> - Open this help dialog
                </div>
                <div style={{ marginBottom: "0.25rem" }}>
                  <kbd>Ctrl+E</kbd> - Export to Excel
                </div>
                <div style={{ marginBottom: "0.25rem" }}>
                  <kbd>1-6</kbd> - Quick navigation between tabs
                </div>
              </div>
            </div>
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              <div style={{ fontSize: "0.875rem", color: "#374151" }}>
                <div style={{ marginBottom: "0.5rem" }}>
                  <strong>Dashboard:</strong> Visual overview with heatmaps and
                  top/bottom performers
                </div>
                <div style={{ marginBottom: "0.5rem" }}>
                  <strong>Fund Scores:</strong> Detailed table of all funds with
                  scores and metrics
                </div>
                <div style={{ marginBottom: "0.5rem" }}>
                  <strong>Class View:</strong> Compare funds within specific
                  asset classes
                </div>
                <div style={{ marginBottom: "0.5rem" }}>
                  <strong>Analysis:</strong> Smart tags and funds requiring
                  review
                </div>
                <div style={{ marginBottom: "0.5rem" }}>
                  <strong>Analytics:</strong> Advanced visualizations including
                  risk-return and correlations
                </div>
                <div style={{ marginBottom: "0.5rem" }}>
                  <strong>History:</strong> Track performance over time and
                  compare snapshots
                </div>
                <div style={{ marginBottom: "0.5rem" }}>
                  <strong>Admin:</strong> Manage recommended funds and benchmark
                  mappings
                </div>
              </div>
            </div>
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              <div style={{ fontSize: "0.875rem", color: "#374151" }}>
                <div style={{ marginBottom: "0.25rem" }}>
                  ...
                  Excellent - Strong risk-adjusted returns and efficiency
                </div>
                <div style={{ marginBottom: "0.25rem" }}>
                  <span style={{ color: "#eab308", fontWeight: "600" }}>
                    50-70:
                  </span>{" "}
                  Good - Average performance, monitor for trends
                </div>
                <div style={{ marginBottom: "0.25rem" }}>
                  <span style={{ color: "#dc2626", fontWeight: "600" }}>
                    Below 50:
                  </span>{" "}
                  Poor - Consider for replacement or further analysis
                </div>
              </div>
            </div>
