...
import AdvisorFilter, { AdvisorFilterLabel } from '../common/AdvisorFilter';
...
function ByAdvisorView() {
  ...
  const [advisorName, setAdvisorName] = useState(''); // Changed to use advisor name instead of ID
  const [consolidatedMode, setConsolidatedMode] = useState(true);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setError(null);
        if (consolidatedMode) {
          const rows = await advisorService.listConsolidatedAdvisorsForDate(date);
          if (cancel) return;
          setAdvisors(rows);
          ...
          if (cancel) return;
          setAdvisors(rows);
          if (rows && rows.length > 0) setAdvisorName(prev => prev || getAdvisorName(rows[0].advisor_id));
        }
      } catch (e) { if (!cancel) setError(e.message || String(e)); }
    })();
    return () => { cancel = true; };
  }, [date, consolidatedMode]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const breakdown = consolidatedMode 
          ? await advisorService.computeConsolidatedPortfolioBreakdown(date, advisorName)
          : await advisorService.computePortfolioBreakdown(date, advisorName); // Use the first advisor ID if individual mode
        if (!cancel) setPortfolio(breakdown || null);
      } catch (e) { if (!cancel) setError(e.message || String(e)); }
      finally { if (!cancel) setLoading(false); }
    })();
    return () => { cancel = true; };
  }, [date, advisorName, consolidatedMode]);
