...
/**
 * Select optimal weighting algorithm based on fund characteristics and user preferences
 */
...
export async function loadEffectiveWeightsResolver() {
  ...
  export function calculateScores(funds, resolverOverride = null) {
    ...
    Object.entries(fundsByClass).forEach(([assetClass, classFunds]) => {
      ...
      fundsWithRawScores.forEach((fund, index) => {
        ...
        if (fund.scoreData.dynamicAdjustments) {
          Object.values(fund.scoreData.dynamicAdjustments).forEach(adj => {
            adjustedBase += adj;
          });
        }
        
        const finalScore = anchors ? scaleScoreRobust(adjustedBase, anchors) : scaleScore(adjustedBase);
        ...
        scoredFunds.push({
          ...
          scores: {
            ...
            confidence: fund.scoreData.confidence,
            dynamicAdjustments: fund.scoreData.dynamicAdjustments,
            weightsSource: fund.scoreData.weightsSource
          }
        });
      });
    });
  ...
  export function calculateScoringConfidence(fund, assetClassPeers = []) {
    ...
    if (fund.standard_deviation_3y != null && fund.standard_deviation_5y != null) {
      const avgStdDev = (fund.standard_deviation_3y + fund.standard_deviation_5y) / 2;
      if (avgStdDev <= 10) {
